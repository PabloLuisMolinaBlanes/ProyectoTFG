...
async function sendData(data : PostData) : Promise<nullable<Test>>   {
    const dataPromise : AxiosResponse<nullable<Test>> = await axios.post(`https://${process.env.HOST_NAME}/upload`, data);
...
async function returnAllExaminations(hospital_name_received: string, hospital_password_received: string) : Promise<nullable<Test[]>>  {
    const dataPromise : AxiosResponse<nullable<Test[]>> = await axios.get(`https://${process.env.HOST_NAME}/receive?hospital_name=${hospital_name_received}&hospital_password=${hospital_password_received}`);
    return dataPromise.data
}
